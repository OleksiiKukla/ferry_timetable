...
class CRUDPort(CRUDBase[Port, PortSchemas, PortSchemas]):
    ...
    async def get_or_create_by_name(self, searching_port_name: str, country_id: int, db: AsyncSession):
        port = await self.get_port_by_name(searching_port_name, db)
        if port is None:
            port = await self.create_port(PortSchemas(
                name=searching_port_name,
